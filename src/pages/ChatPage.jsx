...
const ChatPage = () => {
      ...
      try {
        ...
        if (sellerId) {
          ...
          const existingConv = res.data.find(
            (conv) => conv.user.id.toString() === sellerId
          );
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const sent = await chatAPI.sendMessage(activeConversationId, newMessage);
            ...
            {conversations.map((conv) => (
              <button
                ...
                onClick={() => {
                  ...
                  navigate(
                    `/chat/${
                      conv.participants.find((p) => p.id !== currentUser.id)?.id
                    }/${conv.id}`
                  );
                }}
              >
                <div className="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    {/* <div className="text-muted small">
                      {messages.conversation conv || "No messages yet"}
                    </div> */}
                  </div>
                </div>
              </button>
                  ...
                  {messages.length > 0 ? (
                    messages.map((msg) => (
                      <div
                        key={msg.id}
                        className={`mb-3 ${
                          msg.sender?.id === currentUser.id ? "text-end" : ""
                        }`}
                      >
                        <div
